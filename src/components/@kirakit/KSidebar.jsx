...
function useKSidebar() {
  const context = useContext(KSidebarContext);
  if (!context) {
    throw new Error("useKSidebar must be used within a KSidebarProvider.");
  }
...
const KSidebar = (
  ...
  const { isMobile, state, openMobile, setOpenMobile } = useKSidebar();
    ...
    return (
      <KSheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <KSheetContent
          ...
          side={side}
        >
          <KSheetHeader className="sr-only">
            <KSheetTitle>KSidebar</KSheetTitle>
            <KSheetDescription>Displays the mobile sidebar.</KSheetDescription>
          </KSheetHeader>
...
const KSidebarTrigger = ({ className, onClick, ...props }, ref) => {
  const { toggleKSidebar } = useKSidebar();
...
const KSidebarMenuButton = (
  ...
  const { isMobile, state } = useKSidebar();
